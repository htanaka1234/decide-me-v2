...
1. Load the derived projections and `.ai/decide-me/runtime-index.json` when they exist. Use full
   event-log replay only for `validate-state --full`, conflict detection, or `rebuild-projections`.
...
- `runtime-index.json` checkpoints projection freshness; refresh it with `compact-runtime` or
  regenerate it with `rebuild-projections`.
