...
v2 は **event-sourced runtime** を採用します。つまり、正本は transaction 単位の append-only event files で、`project-state.json`、`session/*.json`、`taxonomy-state.json` はそこから再構成できる派生状態です。
...
* `rebuild-projections` で state を transaction files から再生成できます。
* JSONL transaction files がそのまま provenance になります。
...
`project-state.json` は transaction files から再構成される materialized projection です。人間向け台帳ではなく、**現在の project-visible decision state** を表します。

### 主なフィールド

* `project`: name, objective, current_milestone, stop_rule
* `state`: project_head, event_count, updated_at, last_event_id
...
* `working_state.last_seen_project_head`
...
```json
{
  ...
  "based_on_project_head": "sha256-event-head",
  "is_active": true,
  "activated_at": "...",
...
3. transaction event files と projection の整合性を検証
4. current session がなければ新規作成
5. active proposal を project head と照合し、stale 判定
