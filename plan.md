前提を置きます。現行 Skill は、`SKILL.md` の薄い入口、4本の reference、共有レジスタ、セッション状態、taxonomy、JSON Lines（JSONL）監査ログ、公開 Command-Line Interface（CLI）から成る「状態付き意思決定エンジン」です。互換性を捨てられるなら、これを部分改修するより、**正本の状態と責務分割を整理した v3 をフルスクラッチで実装する方が合理的**です。以下では、現行の中核価値を保持しつつ、重複した責務と legacy の複雑さを削った改訂版実装計画を示します。
...
理由は3点です。
第一に、現行の価値は `YAML→JSON 移行` ではなく、**並行セッション、active proposal、taxonomy-aware search、close summary→plan 変換** にあります。
第二に、現行は `decision-register.json` と `decision-log.jsonl` と `session/*.json` の責務が近接しており、運用規約が強くないと分裂しやすい構造です。
...
| 項目                                        | v3での扱い          | 判断                           |
| ----------------------------------------- | --------------- | ---------------------------- |
| 単一 public skill `decide-me`               | 維持              | 利用者の入口は一つのままにします。            |
| 並行セッション、Resume/Close/Generate plan        | 維持              | 現行 Skill の本質です。              |
| active proposal と plain `OK` 規則           | 維持しつつ改良         | `proposal_id` を追加して曖昧性を下げます。 |
| taxonomy と compatibility tags             | 維持              | 過去セッション再検索のため必須です。           |
| JSONL 監査ログ                                | 維持しつつ正本化        | v3 では event log を正本にします。     |
| `decision-register.json` を共有正本にする設計       | 廃止              | 派生 projection に変更します。        |
| legacy YAML, migration script             | 廃止              | 今回は不要です。                     |
| 4本の公開 CLI                                 | 統合              | 1本の subcommand 型 CLI に統合します。 |
| human-readable register を runtime 正本にする設計 | 廃止              | 人間向け成果物は export に落とします。      |
...
v3 は **event-sourced runtime** を採用します。つまり、正本は append-only の event log で、`project-state.json`、`session/*.json`、`taxonomy-state.json` はそこから再構成できる派生状態です。
...
* 共有レジスタと session file の二重正本問題を避けられます。
...
v3 の正本は、flat な ad hoc JSONL ではなく、**共通 envelope を持つ typed event** にします。
...
現行では log entry が平坦で、event ごとに必須項目が暗黙でした。v3 では envelope を固定し、payload を event_type ごとに schema で検証します。
...
これにより、前回計画で重視した比較・記録の観点を、現行 Skill の runtime に自然に統合できます。
...
現行は `decision_id + recommendation_version` 中心です。v3 では次にします。
...
taxonomy は現行方針をほぼ維持します。これは良い設計です。
...
現行の強みなので維持します。質問前に、次の順で解けないかを見ます。
...
`Decision:` `Recommendation:` `Why:` `If not:` という現行 contract は維持し、`Proposal:` を追加します。
...
これにより、現行の `OK D-012` より曖昧性を下げられます。
...
session close 時には必ず `close_summary` を生成します。ここは現行設計を踏襲しつつ、初版から schema 化します。
...
`classify_session.py` 相当は internal command に統合します。
...
| 1     | repository skeleton                     | `references/`, `schemas/`, `scripts/`, `decide_me/`, `tests/`    | 空の v3 repo が起動可能                                 |
...
* legacy YAML と migration
* 複数の公開 CLI
* hidden state
* runtime 正本としての Markdown / YAML 台帳
...
この v3 は、現行 Skill の価値を保ちながら、正本・派生状態・公開操作・人間向け出力を明確に分離した再設計になっています。最も重要な変化は、**「決定台帳中心」ではなく「event log 中心」の状態付き意思決定エンジンに変えること**です。そうすると、現行で強かった session continuity, taxonomy search, JSONL audit, plan-ready close summary を、より整合的に維持できます。
