前提を置きます。現行 Skill は、`SKILL.md` の薄い入口、4本の reference、共有レジスタ、セッション状態、taxonomy、JSON Lines（JSONL）監査ログ、公開 Command-Line Interface（CLI）から成る「状態付き意思決定エンジン」です。互換性を捨てられるなら、これを部分改修するより、**正本の状態と責務分割を整理した v2 をフルスクラッチで実装する方が合理的**です。以下では、現行の中核価値を保持しつつ、重複した責務と legacy の複雑さを削った改訂版実装計画を示します。
...
現行では log entry が平坦で、event ごとに必須項目が暗黙でした。v2 では envelope を固定し、payload を event_type ごとに schema で検証します。
...
現行は `decision_id + recommendation_version` 中心です。v2 では次にします。
...
この v2 は、現行 Skill の価値を保ちながら、正本・派生状態・公開操作・人間向け出力を明確に分離した再設計になっています。最も重要な変化は、**「決定台帳中心」ではなく「event log 中心」の状態付き意思決定エンジンに変えること**です。そうすると、現行で強かった session continuity, taxonomy search, JSONL audit, plan-ready close summary を、より整合的に維持できます。
