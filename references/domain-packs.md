...
approval posture and required actions. `automatic_adoption` is reported as the effective gate state:
`allowed`, `requires_approval`, or `blocked`. The effective policy also contributes to the gate
digest. Pack-free and `generic` objects keep the core Safety Gate behavior, where critical risk
blocks automatic adoption even if an approval artifact exists.
