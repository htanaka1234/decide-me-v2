...
Blocked gates cannot be satisfied by approval. Gates that need approval become `passed` only when a
matching current approval artifact exists. If evidence, assumptions, risks, or safety-relevant links
change, the gate digest changes and the old approval no longer satisfies the gate.
