...
Draft decision set exports are not produced by the generic Document Compiler. They are generated by
`draft_export.py` from a sidecar DraftDecisionSet, optional `draft-projection.json`, and the current
project head. They follow the same managed Markdown-region discipline, but they are draft-review
artifacts, not canonical document models.
