...
- Install development dependencies with `python3 -m pip install -r requirements-dev.txt` before
  running the full test suite or Phase 11 evaluation suite.
- Run the full test suite with `PYTHONPATH=. python3 -m unittest discover -v`.
- Run the Phase 11 evaluation suite with
...
- `tests/scenarios/`: development-only Phase 11 evaluation fixtures and expected snapshots.
